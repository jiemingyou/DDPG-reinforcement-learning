...
class SandingEnv(core.Env):
    ...
    def __init__(
        ...
        low = np.tile(
            np.ones(2) * self.MIN_SIZE, 1 + self.n_sanding + self.n_no_sanding
        )
        high = np.tile(
            np.ones(2) * self.MAX_SIZE, 1 + self.n_sanding + self.n_no_sanding
        )
    ...
    def collisions_no_sand(self, prev_state):
        ...
        x_nosand, y_nosand = self.get_coords(
            prev_state, offset=2 + 2 * self.n_sanding, n=self.n_no_sanding
        )
        x_dist = x_nosand - x
        y_dist = y_nosand - y
        squared_dist = x_dist * x_dist + y_dist * y_dist
    ...
    def pd_control(
        ...
        for _ in range(num_steps):
            ...
            reward = (
                reward + np.sum(cols) * self.REW_SAND
            )  # Reward depends on number of collisions with sanding circles

            # Move collided sanding circles outside the operating area
            next_state[2 + np.flatnonzero(np.repeat(cols, repeats=2))] = (
                self.MIN_SIZE - 2 * self.RADIUS
            )

            # Collisions with no-sanding circles
            cols = self.collisions_no_sand(prev_state)
            reward = (
                reward + np.sum(cols) * self.REW_NOSAND
            )  # Reward depends on number of collisions with no-sanding circles

            # Move collided no-sanding circles outside the operating area
            next_state[
                2 + 2 * self.n_sanding + np.flatnonzero(np.repeat(cols, repeats=2))
            ] = (self.MIN_SIZE - 2 * self.RADIUS)
    ...
    def step(self, a):
        ...
        if self.is_PD_control:
            ...
            xy_changes, reward, next_state, state_traj = self.pd_control(
                0.08 / 4.5,
                0.1 / 4.5,
                1.2,
                xy,
    ...
    def _render_frame(self, pd_pos, pd_state):
        ...
        for i in range(self.n_sanding):
            ...
            if x > 0 and y > 0:
                ...
                gfxdraw.filled_circle(surf, int(x), int(y), int_radius, (178, 34, 34))
